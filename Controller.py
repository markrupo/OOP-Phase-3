...
class controller:
    ...
    def belegungen_zeigen(self):
        """Gibt Daten vom Service an die View weiter um bestandene Belegungen anzuzeigen"""
        self.view.belegungen(
            len(self.service.student.belegungen),
            self.service.student.belegungen
        ) 

    def alle_module_zeigen(self):
        """Gibt Daten vom Service an die View weiter um alle Module zu zeigen"""
        self.view.print_alle_module(self.service.modul_liste())
    
    def notnenziel_aendern(self):
        ...
        if eingabe == "0":
            ...
            return self.service.noten_ziel_aendern(eingabe)
    
    def zeitziel_aendern(self):
        """Nimmt die Nutzereingabe entgegen und ändert das Zeitziel"""
        eingabe = input("0 = Zurück zum Dashboard\nneues ziel setzen:")
        if eingabe == "0":
            return self.dashboard_zeigen()
        return self.service.zeit_ziel_aendern(eingabe)
    
    def modul_belegen(self):
        ...
        if pruefungstyp == "0":
            return self.dashboard_zeigen()
        os.system('cls' if os.name == 'nt' else 'clear')
        return self.service.pruefung_belegen(modul_index, note, pruefungstyp)
    ...
    def nutzer_eingabe(self):
        ...
        if eingabe == "1":
            ...
            self.belegungen_zeigen()
        elif eingabe == "3":
            self.alle_module_zeigen()
        elif eingabe == "4":
            print(self.modul_belegen())
        elif eingabe == "5":
           print(self.notnenziel_aendern())
        elif eingabe == "6":
            print(self.zeitziel_aendern())
